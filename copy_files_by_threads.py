...
def make_paths(path_from: Path, path_to: Path) -> Path:
    ...
    if full_path.exists():
        full_path = (
            path_to
            / path_from.suffix.removeprefix(".")
            / f"{full_path.stem}_copy{full_path.suffix}"
        )
    return full_path
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2 or len(sys.argv) > 4:
        sys.exit(
            "Arguments must me: first_path second_path threads_count / or first_path second_path"
        )
    ...
    print(
        f"{len(handled_file_paths)} files handled in {time.time() - start_time} seconds with {thread_count} threads"
    )
